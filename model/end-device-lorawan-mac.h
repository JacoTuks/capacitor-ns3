...
public:
  ...
  */
  ...
  bool GetDataRateAdaptation (void);
    ...
    /**
   * Indicate an Application packet was generated by a node participating in ACS.
   */
  void IndicateAppPacketGenerated(Ptr<Packet const> packet, int size);
  ...
  /**
   * Voltage threshold considered by summing the voltage cost of the cycle and
   * the threshold at which the device turns off.
   */
  double GetVoltageThresholdForSuccessfulCycle (Ptr<Packet> packet, Time packetInterval);

  double GetEnergyThresholdForSuccessfulCycle (Ptr<Packet> packet, Time packetInterval);

  /**
   * Initialize Lora Transmission parameters based on set values
   */
  void InitializeLoraTxParams (void);

  /**
   * Return Lora tranmission parameters
   */
  LoraTxParameters GetLoraTxParams (void);

  uint8_t GetReceiveWindowDurationInSymbols (void);
...
protected:
  ...
  /**
   * Sets the dropping of a packet not transmitted because of the DC
   */
  bool m_dropPacketIfDC;
  ...
  /**
   * Threshold above which the MAC transmission is performed
   */
  double m_macMinVoltageTh;

  // /**
  //  * Enable voltage threshold changing according to the current packet size and SF.
  //  */
  // bool m_dynamicMacVoltageTh;
   ...
   */
      TracedCallback<uint8_t, bool, Time, Ptr<Packet>>
          m_requiredTxCallback;

  /**
   * The trace source fired when app Packet is generated.
   *
   * \see class CallBackTraceSource
   */
  TracedCallback<Ptr<Packet const>, int, LorawanMacHeader::MType  > m_appPacketGenerated;
   ...
   */
  TracedCallback<uint32_t, Ptr<const Packet>, Time, bool,  LorawanMacHeader::MType>  m_enoughEnergyForTx;

  /**
   * The trace source fired when a packet can not be sent.
   */
  TracedCallback<Ptr<const Packet>, uint32_t> m_macSendingNotPossible;

  /**
   * The transmission parameters of this device
   */
...
private:
  ...
  /**
   * Compute the voltage cost for this cycle according to the setting (either
   * dynamic or not). It considers packet transmission, idle periods between
   * windows, and a packet of maximum size received in the second receive
   * window, while the first receive window goes empty.
   * If dynamic threshold is true consider the current packet size and SF,
   * otherwise consider a constant component set by the attribute.
   */
  double ComputeMacVoltageCost (Ptr<Packet const> packet, bool dynamicThreshold,
                                Time packetInterval);



  bool CheckEnergy(Ptr<Packet> packet);
